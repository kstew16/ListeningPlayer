...
# mssql 서버 연결을 위한 세팅
def connect_to_mssql_server(driver_name, server_name, database_name, user_name=None, password=None):
    
    if((user_name == None) & (password == None)):
        connection_string = f"""
            DRIVER={{{driver_name}}};
            SERVER={{{server_name}}};
...
def convert_syncs_data(cursor, syncs_list):
...
def convert_extracted_data(cursor, extracted_sync_data):
    ...
    # body - syncs
    converted_syncs_data = convert_syncs_data(cursor, extracted_sync_data)
...
def smi_to_json(cursor, smi_dir_path, smi_file_name, json_dir_path=None, json_file_name=None, search_option="ENCC"):
    ...
    extracted_data = extract_sync_data_from_smi(smi_dir_path, smi_file_name, search_option)

    # 추출한 데이터를 json 형식에 맞게 변환하거나, 추가합니다.
    converted_data = convert_extracted_data(cursor, extracted_data)
...
# smi 폴더 내의 smi 파일 이름 목록과, json 폴더 내의 json 파일 이름 목록 불러옴 (확장자 체크하여 불러옴)
def get_smi_and_json_file_name_list(dir_path):
    
    # smi 폴더 내의 smi 파일 이름 목록 불러옴 (smi 확장자 체크)
    file_list_in_smi_dir = os.listdir(dir_path + "\smi")
        
    smi_file_name_list = []
        
    for file in file_list_in_smi_dir:
        file_name, file_extension = os.path.splitext(file)
        
        if(file_extension != "smi"):
            smi_file_name_list.append(file_name)
                
    # json 폴더 내의 json 파일 이름 목록 불러옴 (json 확장자 체크)
    file_list_in_json_dir = os.listdir(dir_path + "\json")
        
    json_file_name_list = []
        
    for file in file_list_in_json_dir:
        file_name, file_extension = os.path.splitext(file)
        
        if(file_extension != "json"):
            json_file_name_list.append(file_name)
    
    return smi_file_name_list, json_file_name_list

# 변환할 smi 파일들 중 json 폴더 안에 있는 지 확인하여 이미 있으면 작업 목록에서 삭제, 최종적으로 작업할 smi 파일 목록 반환
def except_smi_file_from_task_if_already_converted_to_json(smi_file_name_list, json_file_name_list):
    smi_file_name_list_to_convert = smi_file_name_list.copy()
    
    for smi_file_name in smi_file_name_list:
        if(smi_file_name in json_file_name_list):
            smi_file_name_list_to_convert.remove(smi_file_name)
    
    return smi_file_name_list_to_convert

# smi 폴더 내 모든 smi 파일들을 json으로 변환하여 json 폴더 내에 저장합니다.
def smi_to_json_all(cursor, dir_path, search_option="ENCC"):
    
    # dir_path에 smi 폴더와 json 폴더가 있는 지 확인하고 없으면 생성
    if("smi" not in os.listdir(dir_path)):
        os.mkdir(dir_path + "\smi")
    if("json" not in os.listdir(dir_path)):
        os.mkdir(dir_path + "\json")
        
    # smi 폴더와 json 폴더 경로 지정
    smi_dir_path = dir_path + "\smi"
    json_dir_path = dir_path + "\json"
    
    # smi 폴더 내의 smi 파일 목록과, json 폴더 내의 json 파일 목록 불러옴 (확장자 체크하여 불러옴)
    smi_file_name_list, json_file_name_list = get_smi_and_json_file_name_list(dir_path)
    
    # 변환할 smi 파일들 중 json 폴더 안에 있는 지 확인하여 이미 있으면 작업 목록에서 삭제, 최종적으로 작업할 smi 파일 목록 반환
    smi_file_name_list_to_convert = except_smi_file_from_task_if_already_converted_to_json(smi_file_name_list, json_file_name_list)
        
    ## smi to json 변환
    if(len(smi_file_name_list_to_convert) == 0):
        print("변환할 smi 파일이 없거나, 이미 모두 변환되어 있습니다.")
    else:
        for file_name in smi_file_name_list_to_convert:
            smi_file = file_name + ".smi"
            json_file = file_name + ".json"
            
            print(f">>>>> {smi_file} 변환 중")
            print("")
            
            # smi 파일을 json으로 변환하여 저장합니다.
            try:
                smi_to_json(cursor, smi_dir_path, smi_file, json_dir_path, json_file, search_option)
            except:
                print(f"      {smi_file} 변환 중 오류 발생 !")
                print("")
                logging.error("Exception occurred", exc_info=True)
                print("")
                print("--------------------------------------------------------------------------------------------------")
            else:
                print(f"      {smi_file} 변환 완료")
                print(f"      (저장경로: {json_dir_path}\{json_file})")
                print("--------------------------------------------------------------------------------------------------")    
